...
exports.create = (req, res, next) => {
    ...
    BillRecordControl.createBillRecord(recordbody, req.user.id)
        .then((billrecordcreate) => {
            ...
            // console.log(record);
            const billdata = BillRecordControl.createBillData2(record, databody, req.user.id);
            const billapprove = BillRecordControl.createApproveBill(record, req.user.id)
            // .then(approvebill_rec_id => {
                // })
                // .catch(next);
                Promise.all([billdata, billapprove])
                .then(data => {
                    // console.log(data[0]);
                    const approvebill_rec_id = data[1].id;
                    // console.log()
                    BillRecordControl.findApprover(req.user.id)
                    .then(approverarray =>{
                        // console.log(approverarray[0].parent
