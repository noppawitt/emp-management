...
Recruitment.checkPasswordStatus = cid => (
  db.oneOrNone('SELECT password, lastest_created_password_time, password_lifetimes FROM exam_users WHERE user_id = $1', [cid])
);

Recruitment.assignNewPassword = (cid, newPassword, lifetimes) => (
  db.tx((transaction) => {
