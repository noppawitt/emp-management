...
approvebillrecord.findAll = user_id =>(
    db.manyOrNone('SELECT b.id,b.bill_record_id,b.approvement_1,b.approvement_2,c.child,d.first_name_th,d.last_name_th,e.type_id,f.name '+ 
...
approvebillrecord.findDetail = data =>(
    db.tx(t => {
        const queries = data.map(l => {
            return t.manyOrNone('SELECT * FROM billdata WHERE bill_record_id=$1', [l.billRecordId]);
        });
